...
> **"OpenAI가 추천 결정을 생성하고, 판단 이유를 말할 수 있게 만드는 단계"**
...
Phase 1 추천 결정은 **OpenAI API(Responses API)**를 통해 생성한다.  
규칙 기반 DECIDE-0 로직은 **fallback**으로만 유지하며,  
정상 경로는 모델이 추천과 이유/신뢰도를 직접 생성하는 흐름이다.
